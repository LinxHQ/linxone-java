...
function saveInvoice(){
    $.ajax({
        type: 'POST',
        url: '/api/invoice/delete/'+oldInvoice.id,
        header : {
        ...
        success: function() {
            ...
            $.ajax({
                ...
                success: function(newInvoice) {
                    ...
                    listItem.map((item)=>console.log(item));
                    ...
                    $.ajax({
                        ...
                        success: function(newListItem) {
                            ...
                            $.ajax({
                                ...
                                error: function(jqXHR, textStatus, errorThrown) {
                                    // Fail
                                    alert("Fail create list payment");
                                }
                            });
                        ...
                        error: function(jqXHR, textStatus, errorThrown) {
                            // Fail
                            alert("Fail create list item");
                        }
                    });
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    // Fail
                    alert("Fail create invoice");
                }
            });
        },
        error: function(jqXHR, textStatus, errorThrown) {
            // Fail
            alert("Fail delete invoice");
        }
    });
}
...
window.onload = function(){
    ...
    urlSplit = url.split('/');
    idInvoice = urlSplit[urlSplit.length-1];
...
function render(invoice){
    //render customer
    var idCustomer = invoice.customer.id;
    invoiceCustomer = invoice.customer;
    $("#customerName").empty();
    $("#customerName").append(invoice.customer.name);
    ...
    //render date
    $('#inputDate').val(invoice.date);
    btnSaveDateClick();

    //render term
    var selectTerm;
    var date2 = new Date(invoice.dueDate);
    var date1 = new Date(invoice.date);
    ...
    document.getElementById('selectTerm').selectedIndex=selectTerm;
    btnSaveTermClick();

    //render item
    $.ajax({
        type: 'get',
        url: '/api/invoiceItem/getByIdInvoice/'+invoice.id,
        header : {
            "Content-Type" : "application/json"
        },
        contentType : 'application/json',
        ...
        success: function(items) {
            ...
            for(let j = 0; j < items.length; j++){
                ...
                $("#tblItem").append(
                    '<tr id="item'+index+'">'
                    +'<td><input type="hidden" id="idProductItem'+index+'" value="'+items[j].catalogProduct.id+'"/>'
                    +'<i class="fa fa-trash pt-2" aria-hidden="true" style="float: left;" title="Delete item" onclick="deleteItemByNumber('+index+')"></i>'
                    +'<div class="col-sm-8" style="float: left;">'
                    +'<input type="text" name="" id="inputItemName'+index+'" class="form-control" value="'+items[j].catalogProduct.name+'" required="required" >'
                    +'</div>'
                    +'<a data-toggle="modal" data-target="#itemModal'+index+'">'
                    +'<i class="fa fa-list pt-2" aria-hidden="true" title="Select from list"></i>'
                    +'</a>'
                    ...
                    +'</div>'
                    +'</td>'
                    +'<td><input type="number" name="" id="inputItemQuantity'+index+'" class="form-control" value="'+items[j].quantity+'" required="required" onchange="change();" style="text-align: right"></td>'
                    +'<td><input type="number" name="" id="inputItemUnitPrice'+index+'" class="form-control" value="'+items[j].value+'" required="required"  disabled style="text-align: right"></td>'
                    +'<td><input type="number" name="" id="inputItemTotal'+index+'" class="form-control" value="'+items[j].total+'" required="required"  disabled style="text-align: right"></td>'
                    +'</tr>'
                );
                for (let i = 0; i < listProduct.length; i++){
                    $('#listProduct'+index).append('<tr><td>'+listProduct[i].name+'</td><td>'+listProduct[i].price+'</td><td ><a href="#" onclick="insertProductById('+listProduct[i].id+','+index+')" data-dismiss="modal">insert</a></td></tr>');
                }
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
    ...
    $.ajax({
        type: 'get',
        url: '/api/payment/get-by-id-invoice/'+invoice.id,
        header : {
            "Content-Type" : "application/json"
        },
        contentType : 'application/json',
        ...
        success: function(payments) {
            payments.map((p)=>{
                var dateStr = p.date.slice(0,10);
                paymentLength ++;
                $("#tblPayment").append('<tr id="payment'+paymentLength+'">'
                    +'<td><i class="fa fa-trash" aria-hidden="true" onclick="deletePaymentByNumber('+paymentLength+')"></i></td>'
                    +'<td>'+p.no+'</td>'
                    +'<td><input type="date" name="" id="inputPaymentDate'+paymentLength+'" class="form-control" value="'+dateStr+'" required="required" title=""></td>'
                    +'<td>'
                    +'<select class="form-control" name="" id="selectPaymentMethod'+paymentLength+'" >'
                    +'<option>Master Card</option>'
                    +'<option>Visa Card</option>'
                    ...
                    +'</td>'
                    +'<td>'
                    +'<input type="number" name="" id="inputPaymentAmount'+paymentLength+'" class="form-control" value="'+p.amount+'" required="required" onchange="change();"/>'
                    +'</td>'
                    +'<td>'
                    +'<textarea name="" id="inputPaymentNote'+paymentLength+'" class="form-control" rows="3" required="required" style="height: 50px;"></textarea>'
                    +'</td>'
                ...
                $('#inputPaymentNote'+paymentLength).append(''+p.note);
                ...
                document.getElementById('selectPaymentMethod'+paymentLength).selectedIndex=selectedMethod;
            })
            change();
        },
        error: function(jqXHR, textStatus, errorThrown) {
